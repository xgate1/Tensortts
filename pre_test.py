...
from tensorflow_tts.processor import korspeech
...
def parse_and_config():
    ...
    parser.add_argument(
        "--rootdir",
        # default='D:\Speech\Tacotron2-Wavenet-Korean-TTS-master\datasets\moon\\audio',
        default='D:\Speech\Tacotron2-Wavenet-Korean-TTS-master',
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--outdir",
        default='D:\Face\Tensor_ver\data\son',
        type=str,
        required=False,
    ...
    parser.add_argument(
        "--config", type=str, default='D:\Face\Tensor_ver\TensorflowTTS-master\preprocess\ljspeech_preprocess.yaml',
        required=False, help="YAML format configuration file."
    )
    parser.add_argument(
        "--n_cpus",
        type=int,
...
def gen_audio_features(item, config):
    ...
    assert np.abs(audio).max() <= 1.2, f"{utt_id} is different from 16 bit PCM."
    assert (
...
def save_features_to_file(features, subdir, config):
    ...
    if config["format"] == "npy":
        save_list = [
            (features["audio"], "wavs", "wav", np.float32),
...
def preprocess():
    ...
    processor = dataset_processor[config["dataset"]].Korspeech(
        # config["rootdir"], cleaner_names="english_cleaners"
        config["rootdir"], cleaner_names="korean_cleaners"
    )
...
# def gen_normal_mel(mel_path, scaler):
    ...
    *_, subdir, suffix = path.split(os.sep)
    ...
    np.save(
        os.path.join(
            ...
            # 'D:\\Face\\Tensor_ver\\data\\moon', subdir, "norm-feats", f"{utt_id}-norm-feats.npy"
        ),
...
def normalize(config):
    ...
    glob_path = os.path.join('D:\Face\Tensor_ver\data\moon', "**", "raw-feats", "*.npy")
    ...
    # partial_fn = partial(gen_normal_mel, scaler=scaler)
...
if __name__ == '__main__':
    ...
    dataset_processor = {"moon": korspeech,"son": korspeech, }
    logging.info(f"Selected '{config['dataset']}' processor.")
    processor = dataset_processor[config["dataset"]].Korspeech(
        config["rootdir"], cleaner_names="korean_cleaners"
    )
    # processor = dataset_processor['moon'].Korspeech('D:\Speech\Tacotron2-Wavenet-Korean-TTS-master', cleaner_names="korean_cleaners")
    ...
    # audio, rate = sf.read(l, channels=1)
    # wav,sr=librosa.core.load(l)[0]
    for mel, energy, f0, features in train_map:
        # print('mel:    {}'.format(mel))
        # print('f0:    {}'.format(f0))
        # print('fet:    {}'.format(features))
        save_features_to_file(features, "train", config)
        ...
        try:
            ...
            if len(glob("D:\Face\Tensor_ver\errch.txt"))==0:
                tc = open("D:\Face\Tensor_ver\errch.txt", 'w')
                tc.write(features['utt_id'])
                tc.close()
            else:
                tc = open("D:\Face\Tensor_ver\errch.txt", 'a')
                tc.write(features['utt_id'])
                tc.close()
